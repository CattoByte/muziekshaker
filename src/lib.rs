...
impl State {
    ...
    fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        ...
        let mut thingy = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis();
        let thingy2 = (((thingy % 6283) as f32)/2000.0).sin();
        self.config.width = (thingy2 * 1500.0) as u32 + 1;
        self.surface.configure(&self.device, &self.config);
    }
